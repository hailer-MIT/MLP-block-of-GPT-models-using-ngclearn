...
class SimplePCNMLP:
    ...
    - Residual connection : attention_output + z_mlp2
    ...
    Flow:
    attention_output → [pre-norm] → MLP (z_mlp1 → z_mlp2) → [residual] → [post-norm] → Language Head (z_out)
    ...
    """
    def __init__(self, dkey, input_dim=768, hidden1_dim=3072, vocab_size=50257, 
                 learning_rate=0.001, activation="gelu", T_inference=10, tau_m=10.,
                 use_pre_norm=True, use_residual=True, use_post_norm=False, eps=1e-5):
        """
        Args:
            dkey: JAX random key
            ...
            use_residual: Add residual connection after MLP (default: True)
        ...
        # split keys for components: W1, W2, W_out, E2, E_out
        k1, k2, k3, k4, k5 = random.split(dkey, 5)

        with Context("PCN_MLP_Circuit") as self.circuit:
            # Neuronal layers
            # First hidden MLP layer: stateful expansion layer (d → d_ff)
            # Receives attention output directly (no separate input layer needed)
            self.z_mlp1 = RateCell("z_mlp1", n_units=self.hidden1_dim, tau_m=self.tau_m, act_fx=activation)

            # Second hidden MLP layer: stateful contraction layer (d_ff → d)
            self.z_mlp2 = RateCell("z_mlp2", n_units=self.output_dim, tau_m=self.tau_m, act_fx=activation)

            # Output layer: stateless layer that produces vocabulary logits
            self.z_out = RateCell("z_out", n_units=self.vocab_size, tau_m=0., act_fx="softmax")

            # Error cells (Gaussian errors between prediction mu and actual z)
            self.e1 = GaussianErrorCell("e1", n_units=self.hidden1_dim)
            self.e2 = GaussianErrorCell("e2", n_units=self.output_dim)
            self.e_out = GaussianErrorCell("e_out", n_units=self.vocab_size)

            # Generative synapses: 
            # W1: expands from d to d_ff (predicts z_mlp1 from attention output)
            # W2: contracts from d_ff to d (predicts z_mlp2 from z_mlp1)
            # W_out: projects from d to V (predicts z_out from z_mlp2)
            wlb, wub = -0.1, 0.1
            ...
            self.W2 = HebbianSynapse(
                "W2", shape=(self.hidden1_dim, self.output_dim), eta=learning_rate,
                weight_init=dist.uniform(amin=wlb, amax=wub), bias_init=dist.constant(value=0.),
            ...
            self.E2 = StaticSynapse("E2", shape=(self.output_dim, self.hidden1_dim), 
                                    weight_init=dist.uniform(amin=wlb, amax=wub), key=k4)
            self.E_out = StaticSynapse("E_out", shape=(self.vocab_size, self.output_dim), 
                                       weight_init=dist.uniform(amin=wlb, amax=wub), key=k5)

            # Wiring: generative forward predictions
            # W1: attention output → z_mlp1 (expansion: d → d_ff)
            # Note: W1.inputs will be set directly from attention output in forward()
            self.e1.mu << self.W1.outputs
            self.e1.target << self.z_mlp1.z

            # W2: z_mlp1 → z_mlp2 (contraction: d_ff → d)
            self.W2.inputs << self.z_mlp1.zF
            self.e2.mu << self.W2.outputs
            self.e2.target << self.z_mlp2.z

            # W_out: z_mlp2 → z_out (projection: d → V)
            # Note: W_out.inputs will be updated with normalized/residual MLP output in forward()
            self.e_out.mu << self.W_out.outputs
            self.e_out.target << self.z_out.z
            ...
            # z_mlp2 receives feedback from e_out (via E_out) and its own local error e2
            self.E_out.inputs << self.e_out.dmu
            self.z_mlp2.j << self.E_out.outputs
            self.z_mlp2.j_td << self.e2.dtarget

            # z_out only receives its own local error e_out (no feedback from below)
            self.z_out.j_td << self.e_out.dtarget

            # Hebbian learning signals for synapses
            # W1.pre will be set from attention output in forward()
            self.W1.post << self.e1.dmu

            self.W2.pre << self.z_mlp1.zF
            self.W2.post << self.e2.dmu

            self.W_out.pre << self.z_mlp2.zF
            ...
            def clamp_attention_input(x):
                """Clamp attention block output directly to W1 inputs (no separate input layer)."""
                self.W1.inputs.set(x)
                # Also set W1.pre for learning signal
                self.W1.pre.set(x)
    ...
    def forward(self, attention_output):
        ...
        2. MLP Block: z_mlp1 → z_mlp2
        3. Residual : attention_output + z_mlp2
        4. Post-norm : normalize before language head
        5. Language Head: z_mlp2 → z_out
        ...
        """
        # Store original attention output for residual connection
        ...
        self.circuit.reset()
        self.circuit.clamp_attention_input(attention_output)
        ...
        # Step 3: Get MLP output and apply residual connection
        mlp_output = self.z_mlp2.zF.value
        if self.use_residual:
            mlp_output = residual_input + mlp_output
        ...
        # Step 5: Process through language head with normalized/residual MLP output
        # Update W_out inputs with the processed MLP output
        self.W_out.inputs.set(mlp_output)
        # Also update W_out.pre for learning signal
        self.W_out.pre.set(mlp_output)
        # Advance language head components
    ...
    def train_step(self, attention_output, target_logits=None):
        ...
        # Step 5: Update language head with normalized/residual MLP output
        self.W_out.inputs.set(mlp_output)
        # Also update W_out.pre for learning signal
        self.W_out.pre.set(mlp_output)
        # Advance language head components
        ...
        EFE = self.e1.L.value + self.e2.L.value + self.e_out.L.value
