...
class SimplePCNMLP:
    ...
    def __init__(self, dkey, input_dim=768, hidden1_dim=None, hidden2_dim=None, vocab_size=50257, 
        ...
        with Context("PCN_MLP_Circuit") as self.circuit:
            # Neuronal layers
            # Input (stateless) layer: z_mlp0 holds attention output (no synapse between attention and z_mlp0)
            self.z_mlp0 = RateCell("z_mlp0", n_units=self.input_dim, tau_m=0., act_fx="linear")
            ...
            self.z_mlp_out = RateCell("z_mlp_out", n_units=self.output_dim, tau_m=0., act_fx="linear")

            # Final output layer: stateless layer that produces vocabulary logits
            ...
            # Generative synapses: 
            # W1: expands from d to d_ff (predicts z_mlp1 from attention / z_mlp0)
            # W2: projects from d_ff to d_ff2 (predicts z_mlp2 from z_mlp1)
            # W3: projects from d_ff2 back to d (predicts z_mlp_out from z_mlp2)
            # W_out: projects from d to V (predicts z_out from z_mlp_out)
            wlb, wub = -0.1, 0.1
            ...
            # W1: z_mlp0/attention → z_mlp1 (d → d_ff)
            # Note: W1.inputs will be set from the clamped attention (z_mlp0) in forward()/clamp
            ...
            # z_mlp_out receives feedback from e_out (via E_out) and its own local error e3
            self.E_out.inputs << self.e_out.dmu
            self.z_mlp_out.j << self.E_out.outputs
            self.z_mlp_out.j_td << self.e3.dtarget
            ...
            # W1.pre will be set from attention output (z_mlp0) in forward()/clamp
            ...
            self.W_out.pre << self.z_mlp_out.zF
            ...
            def clamp_attention_input(x):
                """Clamp attention block output to z_mlp0 and W1 inputs (no synapse between them)."""
                # Set input layer state (stateless) and pin W1 inputs from it
                self.z_mlp0.z.set(x)
